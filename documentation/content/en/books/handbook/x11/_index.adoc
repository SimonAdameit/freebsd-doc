...
The driver will create a kernel module, which needs to be loaded at system startup. Add it to `kld_list` in `/etc/rc.conf` by executing:
...
To immediately load the kernel module into the running kernel issue a command like `kldload nvidia-modeset`.
